...
from transwarpnlp.multi_label_classify import model_cnn
...
def train_cnn_classfier(train_path):
    ...
    with tf.Graph().as_default(), tf.Session().as_default() as sess:
        ...
        y_in = tf.placeholder(tf.float32, [None, config.class_num], name="input_y")
        ...
        batch_x_test, batch_y_test = model_cnn.get_test_batch(revs, word_idx_map, config.class_num)

        for i in range(num_steps):
            for minibatch_index in np.random.permutation(range(n_train_batches)):  # 随机打散 每次输入的样本的顺序都不一样
                batch_x, batch_y = model_cnn.generate_batch(revs, word_idx_map, minibatch_index, config.class_num)
...
if __name__ == "__main__":
    ...
    train_cnn_classfier(train_path)
